...
start_url = "https://www.suttonkersh.co.uk/properties/gallery/?FormSearchTextField=&geolat=&geolon=&georad=&section=" \
...
def property_scraper(detail_url_page, property_url):
    ...
    image = detail_url_page.locator(
        "xpath=(//div[@class='container'])[3]").get_attribute("style")
    ...
    guidePrice = detail_url_page.locator("xpath=//a[contains(text(),'Available')]").text_content()
    price, currency = prepare_price(guidePrice)

    description = detail_url_page.locator("xpath=(//div[@class='col-md-8'])[1]").text_content()
    ...
    address = detail_url_page.locator(
        "xpath=//div[@class='col-md-8 col-sm-7']/h1").text_content()
...
def properties_scraper(page, browser):
    auction_divs = []
    anchor_locator = page.locator("//div[@class='img_container']/a")
    ...
    csv_fh = open('sutton_kersh_play.csv', mode='w', newline='', encoding='utf8')
    json_fh = open("sutton_kersh_play.json", "w", encoding='utf8')
    ...
    for property_url in auction_divs:
        ...
        workbook.save("sutton_kersh_play.xlsx")
