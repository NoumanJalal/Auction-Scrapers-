...
start_url = "https://realtime.connectukauctions.co.uk/"


def property_scraper(detail_url_page, property_link):
    detail_url_page.goto(property_link)

    image_link = detail_url_page.locator("//img[@class='main-photo']").get_attribute("src")
    print(image_link)
    ...
    tenure = detail_url_page.locator("(//div[@class='twelve fluid columns']/p)[3]").text_content()
...
def properties_scraper(page, browser):
    ...
    anchor_locator = page.locator("//a[@class='ast-loop-product__link']")

    workbook = openpyxl.Workbook()
    worksheet = workbook.active
    csv_fh = open('connect_auction_play.csv', mode='w', newline='', encoding='utf8')
    json_fh = open("connect_auction_plays.json", "w", encoding='utf8')
    ...
    for property_link in auction_divs:
        ...
        workbook.save("connect_auction_play.xlsx")
