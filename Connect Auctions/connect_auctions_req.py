...
def property_scraper(property_link):
    ...
    image_link = detail_page_result.xpath("(//img[@class='entered lazyloaded'])[1]")[0].attrib["src"]
    print(image_link)

    guidePrice = detail_page_result.xpath("//span[contains(@class,'woocommerce-Price-amount')]")[0].text_content()
    price, currency = prepare_price(guidePrice)

    description_list = []
    sibling_description = detail_page_result.xpath("//div[@id='tab-description']//following-sibling::p")
    ...
    address = detail_page_result.xpath("//div[@class='wprwpg-gmap-content-section']/h2")[0].text_content()

    numbers_of_bedroom = detail_page_result.xpath(
        "(//div[@id='tab-description']/p)[2]")[0].text_content()
    number_of_bedrooms = get_bedroom(numbers_of_bedroom)

    title = detail_page_result.xpath("//h1[contains(@class,'product_title')]")[0].text_content()
    postal_code = parse_postal_code(title)
    property_type = get_property_type(title)

    tenure = detail_page_result.xpath("(//div[@id='tab-description']/p)[5]")[0].text_content()
...
def properties_scraper(url):
    ...
    result_auctions = results.xpath("//a[@class='ast-loop-product__link']")
...
def run():
    url = "https://realtime.connectukauctions.co.uk/"
