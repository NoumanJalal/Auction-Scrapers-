...
def property_scraper(property_link):
    ...
    image_link = detail_page_result.xpath("(//div[@class='slide-content'])[1]")[0].attrib["style"]
    image = re.search(r'url\("?(.+)"?\)', image_link).group(1)
    image_link = image.replace(' " ', " ")
    print(image_link)

    price = detail_page_result.xpath(
        "(//div[@class='col col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12']/p)[3]")[0].text_content()
    price, currency = prepare_price(price)

    address = detail_page_result.xpath(
        "//div[@class='col col-xs-12 col-sm-12 col-md-7 col-lg-7 col-xl-7']//h1")[0].text_content()
    postal_code = parse_postal_code(address)
    print(address)

    description = detail_page_result.xpath("//div[@data-tab-content='tab_key_features']")[0].text_content()

    tenure = detail_page_result.xpath("//p[substring-after(text(),'Fre')]")[0].text_content()
    number_of_bedrooms = get_bedroom(description)

    title = detail_page_result.xpath(
        "(//div[@class='col col-xs-12 col-sm-12 col-md-7 col-lg-7 col-xl-7'])[2]")[0].text_content()
...
def properties_scraper(url):
    ...
    result_page = results.xpath("//a[contains(@class,'property-link')]")
    workbook = openpyxl.Workbook()
    worksheet = workbook.active
    csv_fh = open('taylor_james_req.csv', mode='w', newline='', encoding='utf8')
    json_fh = open("taylor_james_req.json", "w", encoding='utf8')
    ...
    for propertylink in result_page:
        ...
        workbook.save("taylor_james_req.xlsx")
...
def run():
    url = "https://www.taylorjamesauctions.co.uk/auction/online-property-auction-22-03-2023-0800/"
