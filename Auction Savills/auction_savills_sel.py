...
class NetworkAuction:
    ...
    def property_scraper(self, property_link):
        ...
        image = self.driver.find_element(By.XPATH, "(//li[contains(@data-slick-index,'')]/img)[1]")
        image_link = image.get_attribute("src")

        guidePrice = self.driver.find_element(By.XPATH, "//span[contains(text(),'£')]").text
        price, currency = prepare_price(guidePrice)

        description = self.driver.find_element(By.XPATH, "//div[@class='description-paragraph']").text
        number_of_bedrooms = get_bedroom(description)

        address = self.driver.find_element(By.XPATH, "(//div[@class='sv-property-intro__address-block']/div)[1]").text
        postal_code = parse_postal_code(address)

        tenure = get_tenure(description)

        title = self.driver.find_element(
            By.XPATH, "(//div/p)[2]").text
    ...
    def properties_scraper(self):
        ...
        csv_fh = open('auction_savills_sel.csv', mode='w', newline='', encoding='utf8')
        json_fh = open("auction_savills_sel.json", "w", encoding='utf8')
        ...
        for property_link in auction_divs:
            ...
            workbook.save("auction_savills_sel.xlsx")
