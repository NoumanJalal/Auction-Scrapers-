...
start_url = "https://www.venmoreauctions.co.uk/Property-Search?pageNum=1"
...
def property_scraper(detail_url_page, property_link):
    ...
    image_link = detail_url_page.locator(
        "xpath=(//img[@class='img_resp'])[1]").get_attribute("src")
    print(image_link)

    guidePrice = detail_url_page.locator(
        "xpath=//h2[contains(text(),'Guide Price ')]").text_content()
    price, currency = prepare_price(guidePrice)

    description = detail_url_page.locator("xpath=//div[@class='marbot40']").text_content()
    property_type = get_property_type(description)

    short_description = detail_url_page.locator("//div[@class='marbot20']/p").text_content()
    number_of_bedrooms = get_bedroom(short_description)
    tenure = get_tenure(short_description)

    address = detail_url_page.locator(
        "xpath=//span[@class='font-thin f-paragon mini-2']").text_content()
...
def properties_scraper(page, browser):
    auction_divs = []
    anchor_locator = page.locator("//span[contains(@class,'posrel')]/a")
    ...
    csv_fh = open('venmorce_auction_play.csv', mode='w', newline='', encoding='utf8')
    json_fh = open("venmorce_auction_play.json", "w", encoding='utf8')
    ...
    for property_link in auction_divs:
        ...
        workbook.save("venmorce_auction_play.xlsx")
